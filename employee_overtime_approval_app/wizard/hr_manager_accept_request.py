...
class HrManagerAcceptRequest(models.TransientModel):
	_name='hr.manager.accept.request'
	_description ='Hr Manager Accept Request'
	...
	def confirm_button(self):
		...
		if current_id:
			if current_id.hourly_wages == 0.0:
				raise ValidationError(_("Your overtime hours wage not properly added!."))
			template_id = self.env['ir.model.data'].\
				get_object_reference('employee_overtime_approval_app','overtime_request_approve_manager_email_template')[1]
			...
			if template_id:
				...
				values['email_to'] = current_id.employee_id.user_id.partner_id.email or self.env.user.email or '' 
				values['author_id'] = self.env.user.partner_id.id
				values['subject'] = 'Overtime Request For ' + str(current_id.employee_id.name) + ' Confirm'
				body_html = "<p>Dear " + str(current_id.employee_id.name or '') +",<p>" +\
							"<p>Your Overtime Request " + str(current_id.name)+" Confirmed.</p>" +\
							"<p>Regards,</p>"+\
				...
				if msg_id:
					msg_id.sudo().send()
			current_id.hr_confirm_date = self.confirm_date
			current_id.state = 'done'
