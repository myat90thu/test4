...
class KsGlobalDiscountInvoice(models.Model):
    ...
    ks_global_discount_rate = fields.Float('Universal Discount',
                                           ...
                                           states={'draft': [('readonly', False)],
                                                   'sent': [('readonly', False)]})
    ks_amount_discount = fields.Monetary(string='Discount Amount',
                                         readonly=True,
    ...
    amount_after_discount = fields.Monetary(compute='_compute_amount')
    ...
    def ks_calculate_discount(self):
        for rec in self:
            if rec.ks_global_discount_type == "amount":
                ...
                rec.amount_after_discount = rec.amount_untaxed - rec.ks_amount_discount
                rec.amount_tax = sum(t.get('amount', 0.0) for t in
                                     self.invoice_line_ids.mapped('tax_ids').compute_all(rec.amount_after_discount).get(
                                         'taxes', []))
                ...
                if rec.ks_global_discount_rate != 0.0:
                    ...
                    rec.amount_after_discount = rec.amount_untaxed - rec.ks_amount_discount
                    rec.amount_tax = sum(t.get('amount', 0.0) for t in
                                         self.invoice_line_ids.mapped('tax_ids').compute_all(
                                             rec.amount_after_discount).get(
                                             'taxes', []))
                else:
                    rec.ks_amount_discount = 0
                    rec.amount_after_discount = 0
            ...
            if rec.ks_global_discount_rate > 0 and rec.move_type in type_list:
                if rec.is_invoice(include_receipts=True):
                    ...
                    if in_draft_mode:
                        ...
                        tax_line = self.line_ids.filtered(
                            lambda line: line.account_id.user_type_id.name in ('Current Liabilities'))
                        tax_line.update({
                            'debit': self.amount_tax < 0.0 and -self.amount_tax or 0.0,
                            'credit': self.amount_tax > 0.0 and self.amount_tax or 0.0,
                        })
                        ...
                        tax_line = self.line_ids.filtered(
                            lambda line: line.account_id.id == [t.get('account_id') for t in
                                                                self.invoice_line_ids.mapped(
                                                                    'tax_ids').compute_all(
                                                                    rec.amount_after_discount).get(
                                                                    'taxes', [])][0] if len([t.get('account_id') for t in
                                                                self.invoice_line_ids.mapped(
                                                                    'tax_ids').compute_all(
                                                                    rec.amount_after_discount).get(
                                                                    'taxes', [])])>0 else None)
                        ...
                        dict2 = {
                            'debit': self.amount_total > 0.0 and self.amount_total or 0.0,
                            'credit': self.amount_total < 0.0 and -self.amount_total or 0.0,
                        }
                        # dict2 = {
                        #     'debit': total_balance < 0.0 and -total_balance or 0.0,
                        #     'credit': total_balance > 0.0 and total_balance or 0.0,
