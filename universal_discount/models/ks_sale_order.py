...
class KsGlobalDiscountSales(models.Model):
    ...
    ks_global_discount_rate = fields.Float('Universal Discount',
                                           readonly=True,
                                           states={'draft': [('readonly', False)], 'sent': [('readonly', False)]})
    ks_amount_discount = fields.Monetary(string='Discount Amount', readonly=True, compute='_amount_all', store=True,
                                         track_visibility='always')
    ks_enable_discount = fields.Boolean(compute='ks_verify_discount')

    amount_after_discount = fields.Monetary(compute='_amount_all')

    #
    # @api.depends('ks_amount_discount', 'amount_untaxed')
    # def _compute_discount_value(self):
    #     if self.ks_global_discount_rate:
    #         self.amount_after_discount = self.amount_untaxed - self.ks_amount_discount
    #         taxes = self.env['account.tax'].compute_all(self.amount_after_discount)
    #         self.amount_tax = self.amount_after_discount * sum(
    #             t.get('amount', 0.0) for t in taxes.get('taxes', [])) / 100
    #         self.amount_total = self.amount_after_discount + self.amount_tax
    #     else:
    #         self.amount_after_discount = 0.0
        ...
        for rec in self:
            if rec.ks_global_discount_type == "amount":
                ...
                rec.amount_tax = sum(t.get('amount', 0.0) for t in
                                     self.order_line.mapped('tax_id').compute_all(rec.amount_after_discount).get(
                                         'taxes', []))
